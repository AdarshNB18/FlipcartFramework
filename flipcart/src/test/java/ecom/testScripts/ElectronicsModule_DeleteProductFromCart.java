...
public class ElectronicsModule_DeleteProductFromCart extends Base{
	
	@Test
	public void deleteIphoneFromCartTest()
	{
		try {
			Thread.sleep(2000);
		...
		//Step 8:Verify Product added to cart Successfully and delete the product if added
		Reporter.log("Switching control to parent window",true);
		...
		Reporter.log("Finding RemoveProductFromCart Link in CartPage",true);
		cp.getRemoveProductFromCart().click();
		Reporter.log("Found RemoveProductFromCart Link in CartPage and Clicked",true);
		Reporter.log("Finding ConfirmRemovingProduct button",true);
		cp.getConfirmRemovingProduct().click();
		Reporter.log("Found ConfirmRemovingProduct button",true);		
		...
		//Verify Product is successfully Deleted From Cart
		try
		{
		if(cp.getConfirmMobile().isDisplayed())
		{
			Reporter.log("Product not deleted from Cart",true);
		}
		}
		catch(Throwable e)
		{
			Reporter.log("Product Deleted From Cart Successfully",true);
		}
