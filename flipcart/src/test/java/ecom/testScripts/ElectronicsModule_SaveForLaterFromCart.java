...
public class ElectronicsModule_SaveForLaterFromCart extends Base {
	
	@Test
	public void saveIphoneForLaterFromCartTest()
	{
		try {
			Thread.sleep(2000);
		...
		//Step 8:Verify Product added to cart Successfully and Save the product for Later if added
		Reporter.log("Switching control to parent window",true);
		...
		for (int j = 0;  j< 3; j++) 
		{
			try
			{
				isDisplayedCartProduct=cp.getConfirmMobile().isDisplayed();
				break;
			}
			catch(Throwable e)
			{
		...
		Reporter.log("Finding SaveForLater Link in CartPage",true);
		cp.getSaveForLater().click();
		Reporter.log("Found RemoveProductFromCart Link in CartPage and Clicked",true);
		...
		//Verify Product Moved into SavedForLater Category Successfully
		try
		{
		if(cp.getMoveToCart().isDisplayed())
		{
			Reporter.log("Product moved for SaveForLater Category Successfully",true);
		}
		}
		catch(Throwable e)
		{
			Reporter.log("Product not moved for SaveForLater Category Successfully",true);
		}	
