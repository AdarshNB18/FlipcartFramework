...
public class ElectronicsModule_ChangeStorageOfProductThenAddToCart extends Base{
	
	@Test
	public void ChangeIphoneStorageThenAddToCart()
	{
		try {
			Thread.sleep(2000);
		...
		//Step 7:Change Storage of Selected Phone From '64GB' to '256GB'
		Reporter.log("Finding 256GB storage button in Mobile Page",true);
		mp.getStorage().click();
		Reporter.log("Found 256GB storage buttton in Mobile Page",true);
		WebDriverWait wait3=new WebDriverWait(driver, 20);
		wait3.until(ExpectedConditions.titleContains("Apple iPhone XR ( 256 GB Storage, 0 GB RAM ) Online at Best Price On Flipkart.com"));

		//Verify that the Mobile with 256GB storage is Selected
		String expStorageMobile="Apple iPhone XR ( 256 GB Storage, 0 GB RAM ) Online at Best Price On Flipkart.com".replace(" ", "");
		...
		Reporter.log("256GB Storage Mobile is Selected Successfully",true);
		...
		for (int j = 0;  j< 3; j++) 
		{
			try
			{
				isDisplayedCartProduct=cp.getConfirmMobileStorage().isDisplayed();
				break;
			}
			catch(Throwable e)
			{
